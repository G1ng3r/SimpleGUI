...
import ru.sayn.simple_gui.table_model.DisciplinesTableModel
import ru.sayn.simple_gui.table_model.ParticipantsTableModel
...
object SimpleGui extends SimpleSwingApplication {
  ...
  lazy val participants = new Table() {
    ...
    model = new ParticipantsTableModel(rowSet)
    rowHeight = 25
  ...
  lazy val disciplines = new Table() {
    ...
    peer.getTableHeader.setFont(defaultHeaderFont)
    model = new DisciplinesTableModel(rowSet)
    val columnModel = this.peer.getColumnModel()
